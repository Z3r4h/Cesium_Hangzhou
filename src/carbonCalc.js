...
// carbonCalc(tripMode, distance)将返回碳排放量，默认单位为kg
// tripMode指出行方式
// distance指出行距离，单位为公里
...
function carbonCalc(tripMode, distance, unit='kg'){
    ...
    // 私家车/汽油车
    if(tripMode=='car') carbonEmission = 0.192 * distance;
    // 公交车
    else if(tripMode=='bus') carbonEmission = 0.105 * distance;
    // 电力驱动公交车
    else if(tripMode=='electricBus') carbonEmission = 0.09 * distance;
    // 电动汽车
    else if(tripMode=='electricCar') carbonEmission = 0.053 * distance;
    //  自行车 or 步行
    else if(tripMode=='bicycle'||tripMode=='walk') carbonEmission = 0;
